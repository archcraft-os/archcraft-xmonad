...
-- ## Applications ## ---------------------------------------------------------------
-- Terminal
myTerminal      	= "~/.xmonad/bin/terminal"
...
myKeys conf@(XConfig {XMonad.modMask = super}) = M.fromList $
    ...
    , ((super .|. shiftMask, 		xK_Return), spawn "~/.xmonad/bin/terminal -f")
    ...
    -- Brightness keys
    , ((0,         xF86XK_MonBrightnessUp), 	spawn "backlight-up")
    , ((0,         xF86XK_MonBrightnessDown), 	spawn "backlight-down") 

    -- Screenshot
    , ((0, 							xK_Print), 	spawn $ "scrot 'Screenshot_%Y-%m-%d-%I-%M-%S_$wx$h.png' -e 'mv $f $$(xdg-user-dir PICTURES) ; viewnior $$(xdg-user-dir PICTURES)/$f'")
    , ((controlMask, 				xK_Print), 	spawn $ "scrot -d 5 'Screenshot_%Y-%m-%d-%I-%M-%S_$wx$h.png' -e 'mv $f $$(xdg-user-dir PICTURES) ; viewnior $$(xdg-user-dir PICTURES)/$f'" )
    , ((controlMask .|. shiftMask , xK_Print), 	spawn $ "scrot -u -b 'Screenshot_%Y-%m-%d-%I-%M-%S_$wx$h.png' -e 'mv $f $$(xdg-user-dir PICTURES) ; viewnior $$(xdg-user-dir PICTURES)/$f'")
    , ((mod1Mask .|. controlMask  , xK_Print), 	spawn $ "scrot -s 'Screenshot_%Y-%m-%d-%I-%M-%S_$wx$h.png' -e 'mv $f $$(xdg-user-dir PICTURES) ; viewnior $$(xdg-user-dir PICTURES)/$f'")
    ...
    -- Lockscreen
    , ((mod1Mask .|. controlMask, 	xK_l), 		spawn "~/.xmonad/bin/lockscreen")
